<?php
?>
<!DOCTYPE html>
<html lang="fr" dir="ltr">
<head>
    <?php include "../includes/head.php"; ?>
    <title>Titre</title>
    <script src="../js/video.js"></script>
</head>
<body class="flex" onload="removeHUD()">
<div class="side_bar ">
    <img src="../assets/images/icons/ic.png" class="mb-[35px] w-[35px]">
    <img src="../assets/images/icons/logo.png" class="mb-[82px] h-[35px]">
    <img src="../assets/images/icons/home.png" class="mb-[34px] h-[35px]">
    <img src="../assets/images/icons/tranding.png" class="mb-[34px] h-[35px]">
    <img src="../assets/images/icons/explorer.png" class="mb-[34px] h-[35px]">
    <img src="../assets/images/icons/liked.png" class="mb-[34px] h-[35px]">
    <img src="../assets/images/icons/history.png" class="mb-[34px] h-[35px]">

</div>
<div class="center_div h-full   pt-5  pr-12 ml-[80px] ">
    <input type="text" placeholder="Search..."
           class="rounded-3xl pl-6 w-7/12 h-[35px] bg-[#48485B] mb-3 placeholder-white text-sm">
    <video controls class="w-[750px] rounded-2xl ml-8" id="video">
        <source src="../assets/videos/test2.mp4"
                type="video/mp4">
    </video>
    <div class="flex justify-center mt-4">
        <img class=" h-12"
             src="data:image/jpeg;base64,">
    </div>
    <div class="video_info ml-8  pl-20 pt-4">
        <div class="flex justify-between">
            <div class="flex">
                <img src="../assets/images/placeholder/user.jpg" class="h-[35px] rounded-[50%] object-contain mr-2">
                <div>
                    <p class="text-[10px] font-semibold text-white">THESTARTOFUS</p>
                    <p class="text-[9px] font-semibold text-gray-600">129K Subscriber</p>
                </div>
            </div>
            <div class="flex w-1/2 justify-around place-items-center">
                <div class="text-xl flex place-items-center">
                    <ion-icon class="" name="thumbs-up"></ion-icon>
                    1.385
                </div>
                <div class="text-xl flex place-items-center">
                    <ion-icon name="thumbs-down"></ion-icon>
                    15
                </div>
                <div class="text-xl flex">
                    <ion-icon class="text-xl" name="share-social"></ion-icon>
                </div>
                <button class="rounded  border-solid border-[2px] border-[#7764E4] p-1 text-[#7764E4] hover:bg-[#7764E4] hover:text-white">
                    <ion-icon name="trending-up"></ion-icon>
                    Subscribe
                </button>
            </div>
        </div>
        <div class="flex mt-6 justify-between">
            <div class="title_cat w-1/2">
                <h1 class="video_title text-xl  text-gray-300">We Make a Fan's Fantasy a Reality! BJ w/Bent Over
                    Pounding & Cum Filling</h1>

            </div>
            <div class="video_stats border-[1px] border-solid border-[#5E5E78] p-4 rounded-xl">
                <p class="flex">
                    <ion-icon name="eye"></ion-icon>
                    137.897 views
                </p>
                <p class="flex">
                    <ion-icon name="time"></ion-icon>
                    10 Feb 2022 release
                </p>
                <p class="flex">
                    <ion-icon class="text-xl" name="share-social"></ion-icon>
                    103 Shared
                </p>
                <p class="flex">
                    <ion-icon name="cloud-download"></ion-icon>
                    295 download
                </p>


            </div>

        </div>

    </div>
</div>

<div class="right_side h-full 100 w-[460px] pt-5 ">
    <div class="login_buttons flex  place-items-center justify-around w-2/3 mb-6">
        <ion-icon name="grid"></ion-icon>
        <ion-icon name="notifications"></ion-icon>
        <img src="../assets/images/placeholder/user.jpg" class="h-[25px] rounded-[50%] object-contain mr-2">
        <p>Aurelien K</p>
        <ion-icon name="chevron-down"></ion-icon>
    </div>

    <img src="../assets/images/placeholder/ad.png" class="w-1/2 mb-4 ml-10">

    <div class="suggestions w-4/5  bg-[#48485B] rounded-xl p-4">
        <h1 class="mb-2">Suggestions</h1>
        <div class="flex">
            <img src="../assets/images/placeholder/video.png" class="w-1/2">
            <div class="w-1/2 p-2 flex flex-col justify-between">
                <p class="text-white text-[12px]">Sexy lingerie leads to intense and</p>
                <p class="text-gray-400 text-[10px]">THESTARTOFUS</p>
                <div class="flex justify-between">
                    <p class="text-gray-500 text-[10px] font-semibold">150.985 view</p>
                    <p class="text-gray-500  text-[10px] font-semibold">●</p>
                    <p class="text-gray-500 text-[10px] font-semibold">10 Jan 2022</p>
                </div>

            </div>
        </div>

    </div>

</div>
<script>

</script>

</body>
</html>
