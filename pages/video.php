<?php
?>
<!DOCTYPE html>
<html lang="fr" dir="ltr">
<head>
    <?php include "../includes/head.php"; ?>
    <title>Titre</title>
    <script src="../js/video.js"></script>
</head>
<body class="flex flex-col md:flex-row " onload="removeHUD()">

<?php include '../includes/sidebar.php' ?>

<div class="center_div h-full xl:w-[1180px] lg:w-full pt-0 sm:pt-5  md:pr-12 md:ml-0 lg:ml-[80px] ">
    <div class="responsive_header h-12 p-4 w-full bg-[#262633] mb-4 flex justify-between place-items-center sm:hidden">
        <h1 class="text-xl font-semibold text-[#70708F]">NomDuSite</h1>
        <ion-icon class="text-3xl text-[#70708F]" name="menu"></ion-icon>
    </div>


    <input type="text" placeholder="Search..."
           class="rounded-3xl pl-6 lg:w-9/12 xl:7/12 sm:w-11/12 w-9/12 h-12 bg-[#48485B] mb-3 placeholder-white text-sm ml-12">
    <video controls class=" rounded-2xl md:ml-2 lg:ml-8" id="video">
        <source src="../assets/videos/test.mp4"
                type="video/mp4">
    </video>
    <div class="flex justify-center mt-4">
        <img class=" lg:h-16 md:h-12"
             src="data:image/jpeg;base64,">
    </div>
    <div class="video_info ml-8 mr-8  lg:pl-4 xl:pl-20 pt-4">
        <div class="flex justify-between flex-col lg:flex-row">
            <div class="flex place-items-center">
                <img src="../assets/images/placeholder/user.jpg" class=" h-12 rounded-[50%] object-contain mr-2">
                <div>
                    <p class="text-xl font-semibold text-white">Amixem</p>
                    <p class="text-sm font-semibold text-gray-600">129K Subscriber</p>
                </div>
            </div>
            <div class="flex xl:w-2/5  lg:w-3/5 justify-around place-items-center text-2xl">
                <div class=" flex place-items-center">
                    <ion-icon class="" name="thumbs-up"></ion-icon>
                    1.385
                </div>
                <div class=" flex place-items-center">
                    <ion-icon name="thumbs-down"></ion-icon>
                    15
                </div>
                <div class=" flex">
                    <ion-icon class="" name="share-social"></ion-icon>
                </div>
                <button class="rounded  border-solid border-[2px] border-[#7764E4] p-1 text-[#7764E4] hover:bg-[#7764E4] hover:text-white">
                    <ion-icon name="trending-up"></ion-icon>
                    Subscribe
                </button>
            </div>
        </div>
        <div class="flex mt-2 justify-between flex-col lg:flex-row">
            <div class="title_cat lg:w-1/2 md:w-full">
                <h1 class="video_title text-2xl  text-gray-300">LES PIRES LIFEHACKS TITKTOK (ça va changer votre
                    vie)(pas sur)</h1>

            </div>
            <div class="video_stats border-[1px] border-solid border-[#5E5E78] p-4 rounded-xl text-2xl mt-2 mr-0 md:mr-4 md:mr-0">
                <p class="flex place-items-center ">
                    <ion-icon class="mr-2" name="eye"></ion-icon>
                    137.897 views
                </p>
                <p class="flex place-items-center">
                    <ion-icon class="mr-2" name="time"></ion-icon>
                    10 Feb 2022 release
                </p>
                <p class="flex place-items-center">
                    <ion-icon class="mr-2" name="share-social"></ion-icon>
                    103 Shared
                </p>
                <p class="flex place-items-center">
                    <ion-icon class="mr-2" name="cloud-download"></ion-icon>
                    295 download
                </p>


            </div>

        </div>

    </div>
</div>

<div class="right_side h-full  pt-5 ">
    <div class="login_buttons   place-items-center justify-around w-2/3 mb-6 hidden md:flex">
        <ion-icon name="grid"></ion-icon>
        <ion-icon name="notifications"></ion-icon>
        <img src="../assets/images/placeholder/user.jpg" class="h-10 rounded-[50%] object-contain mr-2">
        <p>Aurelien K</p>
        <ion-icon name="chevron-down"></ion-icon>
    </div>

    <img src="../assets/images/placeholder/ad.png" class=" w-3/5 mb-4 ml-10 hidden md:block" alt="">

    <div class="suggestions bg-[#48485B]  rounded-xl p-4">
        <h1 class="mb-2 text-xl  font-semibold">Suggestions</h1>
        <div class="flex md:w-[325px] sm:w-[270px] ">
            <img src="../assets/images/placeholder/video.png" class="w-1/2 object-contain">
            <div class="w-1/2 p-2 flex flex-col justify-between">
                <p class="text-white font-semibold  ">Ceci est une deuxieme vidéo</p>
                <p class="text-gray-400 text-sm">Utilisateur deux</p>
                <div class="flex justify-between">
                    <p class="text-gray-500 text-sm  font-semibold">150.985 view</p>
                    <p class="text-gray-500 text-sm   font-semibold">●</p>
                    <p class="text-gray-500 text-sm  font-semibold">10 Jan 2022</p>
                </div>

            </div>
        </div>

    </div>

</div>
<script>

</script>

</body>
</html>
